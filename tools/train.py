...
from src.dataset import collate_fn, meta_collate_fn, SubsetSequentialSampler, SubsetRandomSampler
...
def main():
    ...
    optimizer = optim.SGD(model.parameters(), lr=cfg.OPTIMIZATION.META_LR, momentum=0.9)
    ...
    for al_type, index_lists in labeled_index_lists.items():
        for step, l_index in enumerate(index_lists):
            labeled_dataloader = DataLoader(train_dataset,
                                            batch_size=cfg.OPTIMIZATION.BATCH_SIZE,
                                            sampler=SubsetRandomSampler(l_index),
                                            collate_fn=collate_fn)
            model = Learner(cfg.MODEL)
            optimizer = optim.SGD(model.parameters(), lr=cfg.OPTIMIZATION.META_LR, momentum=0.9)
            model = train(cfg.OPTIMIZATION, model, optimizer, labeled_dataloader, tb_log=tb_log)
            acc_list = test_classes(model, test_dataloader, tb_log)(model, test_dataloader, tb_log=tb_log)
            tb_log.add_scalars('acc', {al_type: acc}, global_step=step)
